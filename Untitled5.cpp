...
void div(bignum_t a,const int b,int& c){
    int i;
    int tt=0;
    for (tt=0,i=a[0];i;tt=tt*DEPTH+a[i],a[i]=tt/b,tt%=b,i--);
    c=tt;
    for (;!a[a[0]]&&a[0]>1;a[0]--);
}
...
public:
    ...
    inline int operator>=(const bignum& a){return sgn>0?(a.sgn>0?comp(num,a.num)>=0:1):(sgn<0?(a.sgn<0?comp(num,a.num)<=0:0):a.sgn<=0);}
    inline int operator>=(const int a){return sgn>0?(a>0?comp(num,a)>=0:1):(sgn<0?(a<0?comp(num,-a)<=0:0):a<=0);}
    inline bool operator<(const bignum& a)const{return sgn<0?(a.sgn<0?comp(num,a.num)>0:1):(sgn>0?(a.sgn>0?comp(num,a.num)<0:0):a.sgn>0);}
    inline int operator<(const int a){return sgn<0?(a<0?comp(num,-a)>0:1):(sgn>0?(a>0?comp(num,a)<0:0):a>0);}
    inline int operator<=(const bignum& a){return sgn<0?(a.sgn<0?comp(num,a.num)>=0:1):(sgn>0?(a.sgn>0?comp(num,a.num)<=0:0):a.sgn>=0);}
    inline int operator<=(const int a){return sgn<0?(a<0?comp(num,-a)>=0:1):(sgn>0?(a>0?comp(num,a)<=0:0):a>=0);}
...
pair<bignum, bignum> add(pair<bignum, bignum> a, pair<bignum, bignum> b)
{
    pair<bignum, bignum> ret;
    ret.second = a.second * b.second;
    ret.first = a.first * b.second + a.second * b.first;
    bignum g = GCD(ret.second, ret.first);
    ret.second /= g;
    ret.first /= g;
    return ret;
}

pair<bignum, bignum> mul(pair<bignum, bignum> a, pair<bignum, bignum> b)
{
    pair<bignum, bignum> ret;
    ret.second = a.second * b.second;
...
bignum exp_mod(bignum a, bignum b, bignum p) {
    bignum res;
    res = 1;
    while(b != 0) {
...
bignum Comb(bignum a, bignum b,bignum p) {
    if(a<b)   return 0;
    if(a == b)  return 1;
    if(b > a - b)   b = a - b;
...
bignum Lucas(bignum n, bignum m, bignum p) {
     bignum ans;
     ans=1;
